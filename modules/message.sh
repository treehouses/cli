function message {
  ...
  function get_channel_slack {
    ...
    echo $?
	echo 2
    channel_list=$(curl -s -F token=$access_token -F types=public_channel,private_channel https://slack.com/api/users.conversations; echo $?)
	echo 3
    channels=$(echo $channel_list | python -m json.tool | jq '.channels[].name' | tr -d '"'; echo $?)
	echo 4
    user_list=$(curl -s -F token=$access_token https://slack.com/api/users.list; echo $?)
	echo 5
    users=$(echo $user_list | python -m json.tool | jq '.members[].name' | tr -d '"'; echo $?)
	echo 6
