...
@test "$clinom services planet up" {
  ...
  assert_success
  assert_output -p 'planet built and started'
}
...
@test "$clinom services running full" {
  ...
  assert_success
  assert_output -p 'treehouses/planet'
}
