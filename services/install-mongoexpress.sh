...
function install {
  ...
  # create yml(s)
  {
    ...
    echo "      - 8090:8081"
    echo "    environment:"
    echo "      - ME_CONFIG_MONGODB_ADMINUSERNAME:\${ME_CONFIG_MONGODB_ADMINUSERNAME_VAR}"
    echo "      - ME_CONFIG_MONGODB_ADMINPASSWORD:\${ME_CONFIG_MONGODB_ADMINPASSWORD_VAR}"
  } > /srv/mongoexpress/mongoexpress.yml

  # create .env with default values
  {
    echo "MONGO_INITDB_ROOT_USERNAME_VAR=root"
    echo "MONGO_INITDB_ROOT_PASSWORD_VAR=example"
    echo "ME_CONFIG_MONGODB_ADMINUSERNAME_VAR=root"
    echo "ME_CONFIG_MONGODB_ADMINPASSWORD_VAR=example"
  } > /srv/mongoexpress/.env

  # add autorun
  {
    echo "mongoexpress_autorun=true"
    echo
    echo "if [ \"\$mongoexpress_autorun\" = true ]; then"
    echo "  docker-compose -f /srv/mongoexpress/mongoexpress.yml -p mongoexpress up -d"
    echo "fi"
    echo
    echo
  } > /srv/mongoexpress/autorun
}
