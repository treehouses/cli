function wifi {
  ...
  rm -rf /etc/udev/rules.d/90-wireless.rules
  {
    ...
    wificountry="US"
    if [ -r /etc/rpi-wifi-country ]; then     
...
function wifi_help {
  ...
  echo 
  echo "Usage: $BASENAME wifi <ESSID> [password] hidden"
  echo
  ...
  echo
  echo "  $BASENAME wifi home homewifipassword hidden"
  echo
  echo "      Connects to a hidden wifi network named 'home' with password 'homewifipassword'."
  echo
  echo "  $BASENAME wifi yourwifiname hidden" 
  echo
