...
function tor {
  ...
  if [ "$1" = "list" ]; then
    ...
    if ! grep -wq "HiddenServicePort $2" /etc/tor/torrc ; then
      log_and_exit0 "Port $2 is not assigned"
    fi
    ...
    restart_service tor
    logit "All ports have been deleted"
  elif [ "$1" = "stop" ]; then
    stop_service tor
    logit "Success: the tor service has been stopped"
  elif [ "$1" = "start" ]; then
    ...
    start_service tor
    logit "Success: the tor service has been started"
  elif [ "$1" = "destroy" ]; then
    stop_service tor
    echo > /etc/tor/torrc
    rm -rf /var/lib/tor/treehouses

    logit "Success: the tor service has been destroyed"
  elif [ "$1" = "notice" ]; then
    option="$2"
    if [ "$option" = "on" ]; then
